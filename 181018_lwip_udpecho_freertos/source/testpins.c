...
void PIN_config(void)
{
	...
	PORT_SetPinConfig(PORTC, 3, &config_pin);
	PORT_SetPinConfig(PORTC, 2, &config_pin);
	...
	GPIO_PinInit(GPIOC, 3, &pin_config_gpio); // para FTM Handler
	GPIO_PinInit(GPIOC, 2, &pin_config_gpio); // para monitorear despertar dormir de tarea UDP_server
